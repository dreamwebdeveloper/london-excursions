(() => {
    ...
    function activeBtn() {
        for (i = 0; i <= btn.children.length - 1; i++) btn.children[i].querySelector(".excursions__tabs-btn").classList.remove("active");
        this.classList.add("active");
    }
    function myapp() {
        const buttons = document.querySelectorAll(".excursions__tabs-btn");
        const cards = document.querySelectorAll(".excursions__item");
        function filter(category, items) {
            items.forEach((item => {
                const isItemFiltered = !item.classList.contains(category);
                const isShowAll = category.toLowerCase() === "all";
                if (isItemFiltered && !isShowAll) item.classList.add("hide"); else item.classList.remove("hide");
            }));
        }
    ...
    const activeFixedBtn = function() {
        if (window.scrollY >= 1e3) fixedBtn.classList.add("active"); else fixedBtn.classList.remove("active");
    };
    ...
    if (contacts) {
        const openContacts = contacts.querySelector(".contacts-open");
        const closeContacts = contacts.querySelector(".contacts-close");
        openContacts.addEventListener("click", (e => {
            e.preventDefault();
            openContacts.classList.toggle("active");
